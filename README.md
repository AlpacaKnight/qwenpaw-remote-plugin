...
如果需要避免浏览器缓存旧的前端 bundle，应由 QwenPaw Web 宿主在加载插件前端时根据插件版本自动追加 cache-busting 参数，例如把 `ui/dist/index.js` 加载为 `ui/dist/index.js?v=0.1.4`。插件包内的 `plugin.json` 仍应保持裸路径，避免把版本号写死到入口路径里。
...
```bash
docker cp dist/qwenpaw-remote-plugin-0.1.4.zip copaw:/tmp/qwenpaw-remote-plugin-0.1.4.zip
docker exec copaw sh -lc '/app/venv/bin/qwenpaw plugin install /tmp/qwenpaw-remote-plugin-0.1.4.zip --force'
```
